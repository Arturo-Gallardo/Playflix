...
export const CoverCard = memo(function CoverCard({
  ...
  const isArtLoaded =
    cover.albumArtUrl !== null && loadedArtUrl === cover.albumArtUrl;
      ...
      {cover.albumArtUrl ? (
        <img
          ...
          className={`absolute inset-0 size-full object-cover transition duration-200 group-hover:scale-105 group-hover:opacity-65 ${
            isArtLoaded ? "opacity-85" : "opacity-0"
          }`}
          decoding="async"
          loading="lazy"
          onLoad={() => setLoadedArtUrl(cover.albumArtUrl)}
          src={cover.albumArtUrl}
        />
      ) : null}
      <span className={getOverlayClassName(isArtLoaded)} />
...
function getOverlayClassName(isArtLoaded: boolean) {
  if (!isArtLoaded) {
    return "absolute inset-0 bg-white/[0.04] transition";
  }

  return "absolute inset-0 bg-black/15 transition group-hover:bg-[#1DB954]/20";
}
