...
import { hydratePlaylistCover, toPlaylistCoverWire } from "./cover-canvas";
import type { CanvasTile, Point } from "./canvas-layout";
import type { PlaylistCoverWire } from "../data/sample-covers";

export const tileClipboardMarker = "playflix-tile-clipboard-v1";
...
export function buildTileClipboardPayload(
  ...
  return {
    ...
    tiles: selectedTiles.map((tile) => ({
      ...
      cover: toPlaylistCoverWire(tile.cover),
    })),
  };
}
...
function buildPastedTileEntriesAtOrigin(
  ...
  return payload.tiles.map((entry) => ({
    cover: hydratePlaylistCover(entry.cover),
...
function isTileClipboardEntryWire(
  ...
  return (
    ...
    typeof value.cover.id === "string" &&
    typeof value.cover.title === "string" &&
    typeof value.cover.url === "string"
  );
}
