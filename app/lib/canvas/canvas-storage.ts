...
export const canvasLayoutStorageKey = "playlix:canvas-layout";
...
function isCanvasSnapshotWireV1(value: unknown): value is CanvasSnapshotWireV1 {
  return (
    ...
    typeof value.savedAt === "number" &&
    Array.isArray(value.covers) &&
    value.covers.every(isPlaylistCoverWire) &&
    Array.isArray(value.tiles) &&
    value.tiles.every(isCanvasTileLayoutWire) &&
    Array.isArray(value.movedTileIds) &&
    value.movedTileIds.every((tileId) => typeof tileId === "string") &&
...
function isPlaylistCoverWire(value: unknown): value is PlaylistCoverWire {
  return (
    isRecord(value) &&
    typeof value.id === "string" &&
    typeof value.title === "string" &&
