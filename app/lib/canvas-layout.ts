...
export function createBatchTileId(batchId: string, coverId: string) {
...
export function resolvePastedTileId(
  ...
  const primaryId = createBatchTileId(batchId, coverId);

  if (!occupiedTileIds.has(primaryId)) {
    return primaryId;
  }

  const indexedId = createBatchTileId(batchId, `${coverId}~${entryIndex}`);
  ...
  return createBatchTileId(batchId, `${coverId}~${crypto.randomUUID()}`);
}
