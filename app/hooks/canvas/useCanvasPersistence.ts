...
type UseCanvasPersistenceOptions = {
  ...
  getCoversForSnapshot: () => CanvasCoverWire[];
  hasTiles: boolean;
  onCanvasClear?: () => void;
...
export function useCanvasPersistence({
  ...
  const [hasFinishedInitialRestore, setHasFinishedInitialRestore] =
    useState(false);
  ...
  const persistCanvas = useCallback(
    (shouldNotifyManualSave: boolean, force = false) => {
      ...
      const snapshotVideos = getCoversForSnapshot();

      const didSave = writeCanvasSnapshot(
        buildCanvasSnapshot({
          covers: snapshotVideos,
          tiles,
          movedTileIds,
    ...
    [getLayoutSnapshot, getLiveCamera, getPlaylistSources, getCoversForSnapshot],
  );
  ...
  const captureClearUndoCheckpoint = useCallback(() => {
    ...
    bumpClearUndo();
  }, [bumpClearUndo, getLayoutSnapshot, getLiveCamera, getPlaylistSources, getCoversForSnapshot]);
  ...
  const undoClear = useCallback(() => {
    ...
    restoreTileState(checkpoint.covers, {
  ...
  useLayoutEffect(() => {
    ...
    if (!snapshot || snapshot.covers.length === 0) {
      isRestoringRef.current = false;
      setHasFinishedInitialRestore(true);
      return;
    }

    setPlaylistSources(snapshot.playlistSources);
    restoreTileState(snapshot.covers, {
  ...
  const buildCurrentCanvasSnapshot = useCallback((): CanvasSnapshotWire | null => {
    if (!hasTilesRef.current) {
      return null;
    }
  ...
  const importCanvasLayout = useCallback(
    (snapshot: CanvasSnapshotWire): boolean => {
