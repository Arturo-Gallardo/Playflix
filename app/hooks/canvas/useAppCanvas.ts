...
export function useAppCanvas(initialCovers: PlaylistCover[] = []) {
  ...
  const handleFocusCoverTile = useCallback(
    (tileId: string) => {
      if (!viewportSize) {
        return;
      }
  ...
  const handleClearPlaylistCache = useCallback(() => {
    const clearedCount = clearAllPlaylistCoverCache();
  ...
  const handleCanvasImport = useCallback(
    async (file: File) => {
      ...
      const tileLabel = snapshot.covers.length === 1 ? "tile" : "tiles";
      showNotification(`imported ${snapshot.covers.length} ${tileLabel}`);
    },
    [
      importCanvasLayout,
      setCenterPlaylistRequest,
