...
import {
  findTilesByCoverQuery,
  getListedCoverSearchMatches,
} from "../../lib/canvas/cover-search";

type UseCoverSearchOptions = {
  onFocusTile: (tileId: string) => void;
  tiles: CanvasTile[];
};

export function useCoverSearch({ onFocusTile, tiles }: UseCoverSearchOptions) {
  const [query, setQuery] = useState("");
  const [activeMatchIndex, setActiveMatchIndex] = useState(0);
  const [isOpen, setIsOpen] = useState(false);

  const matches = useMemo(
    () => findTilesByCoverQuery(tiles, query),
    [query, tiles],
  );

  const { listed, overflowCount } = useMemo(
    () => getListedCoverSearchMatches(matches),
    [matches],
  );
