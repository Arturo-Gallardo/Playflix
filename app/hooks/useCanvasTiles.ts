...
import { hydrateCanvasCover, toCanvasCoverWire } from "../lib/cover-canvas";
import {
  areTileLayoutCheckpointsEqual,
  createTileLayoutCheckpoint,
  type TileLayoutCheckpoint,
} from "../lib/tile-layout-checkpoint";
import type {
  CanvasSnapshotWire,
  CanvasCoverWire,
...
export function useCanvasTiles({
  ...
  const getCoversForSnapshot = useCallback((): CanvasCoverWire[] => {
    return tileStateRef.current.tiles.map((tile) =>
      toCanvasCoverWire(tile.id, tile.cover),
    );
  }, []);
  ...
  const captureCurrentCheckpoint = useCallback((): TileLayoutCheckpoint => {
    return createTileLayoutCheckpoint({
      ...
      covers: tileStateRef.current.tiles.map((tile) =>
        toCanvasCoverWire(tile.id, tile.cover),
      ),
    });
  }, [getPlaylistSources]);
  ...
  const applyTileLayoutCheckpoint = useCallback(
    (checkpoint: TileLayoutCheckpoint) => {
      setTileStateWithRef(
        buildTileStateFromSnapshot(checkpoint.covers, {
  ...
  const appendCoverTiles = useCallback(
  ...
  const insertTilesAtPositions = useCallback(
    (
      entries: Array<{ cover: PlaylistCover; x: number; y: number }>,
      batchId: string,
    ) => {
      if (entries.length === 0) {
        return [] as string[];
